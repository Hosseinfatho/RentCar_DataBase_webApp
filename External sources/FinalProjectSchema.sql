...
-- 7. RENT Table
CREATE TABLE RENT (
    RENTID SERIAL,  -- Changed from INT to SERIAL for auto-increment
    DATE DATE,
    PRIMARY KEY (RENTID),
    ...
    FOREIGN KEY (NAME) REFERENCES DRIVER(NAME), -- Added (NAME) for clarity
    FOREIGN KEY (EMAIL) REFERENCES CLIENT(EMAIL), -- Added (EMAIL) for clarity
...
CREATE TABLE REVIEW (
    RATING INT,
    REVIEWID SERIAL,  -- Changed from INT to SERIAL for auto-increment
    MESSAGE VARCHAR(500),
    NAME VARCHAR(100),
    PRIMARY KEY (REVIEWID), -- Changed PK to just REVIEWID (more standard)
    EMAIL VARCHAR(100) NOT NULL,
    FOREIGN KEY (EMAIL) REFERENCES CLIENT(EMAIL), -- Added (EMAIL) for clarity
    FOREIGN KEY (NAME) REFERENCES DRIVER(NAME) -- Added (NAME) for clarity
);
